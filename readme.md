...
---
## ✅ Phase 7: Request FIFO
**RTL:** `rtl/request_fifo.sv`  
**Testbench:** `tb/tb_request_fifo.sv`

### Description
Implemented a synchronous First-In-First-Out (FIFO) buffer to temporarily store SDRAM read and write requests generated by the host processor. The FIFO decouples the request generation rate from the SDRAM controller execution rate and prevents request loss when the controller is busy.

---

### Incentive
SDRAM transactions require multiple clock cycles due to initialization, refresh operations, row activation, and protocol timing constraints. The processor may generate requests faster than the controller can service them.

A request FIFO provides buffering between the processor and the SDRAM controller by:

- Absorbing bursts of incoming requests
- Preserving request ordering
- Preventing request loss while the controller is busy
- Decoupling the processor interface from SDRAM timing

---

### Request Format

Each FIFO entry stores one complete memory request:

```text
+------+-----------+------------+
| rw   | address   | write_data |
+------+-----------+------------+
| 1bit | 22 bits   | 16 bits    |
+------+-----------+------------+
```
...
- `clk` : System clock
- `rst` : Active-high reset
- `push` : Inserts a new request into the FIFO
- `pop` : Removes the oldest request from the FIFO
- `rw_in` : Read/Write indicator
- `addr_in[21:0]` : Linear memory address
- `wdata_in[15:0]` : Write data

#### Outputs
- `rw_out` : Read/Write indicator of the oldest request
- `addr_out[21:0]` : Address of the oldest request
- `wdata_out[15:0]` : Write data of the oldest request
- `full` : FIFO is full
- `empty` : FIFO is empty
...
- Storage Width : 39 bits per entry
- Circular buffer implementation
- Separate write and read pointers
- Entry counter for generating status flags

Internal registers:

```text
mem[0:3]   → Request storage
...
- Reset operation
- Single request insertion
- Data integrity after insertion
- Request removal
...
[PASS] FIFO data verified
[PASS] Pop operation successful
...
The Read/Write FSM will fetch requests from the FIFO and execute the required SDRAM transaction sequence.
